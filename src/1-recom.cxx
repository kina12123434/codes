...
int Recom::mf_pred(std::string dir, double K_percent, double beta, double alpha, int steps) //K以外は初期値有なので指定無しでも可
{
  ...
  int mf_seed = 0;
  double qsum,psum;//初期値確認
  double qsum_v[10],psum_v[10];
  // trialLimit = 1; //debug
  for(int rand_mf_trial = 0; rand_mf_trial < trialLimit; rand_mf_trial++){
    ...
    //以下3行　初期値確認
    psum_v[rand_mf_trial] = psum;
    qsum_v[rand_mf_trial] = qsum;
    //std::cout << "P:\n" << psum << "\nQ:\n" << qsum << std::endl;
...
/*
std::cout << "Q_r_c:" << Q.rows()<< " " << Q.cols()<< std::endl; 
    std::cout << "Q_numerator_r_c:" << Q_numerator.rows()<< " " << Q_numerator.cols()<< std::endl; 
    std::cout << "Q_denominator_r_c:" << Q_denominator.rows()<< " " << Q_denominator.cols()<< std::endl; 
    */

    for(int row=0;row<Q.rows();row++){
      for(int col=0;col<Q.cols();col++){
        //std::cout <<Q_numerator[row][col]<< " "<< Q_denominator[row][col] << std::endl; 
        //std::cout << "row:"<< row <<" col:"<< col<<" Q_denominator:"<<  Q_denominator[col][row] << std::endl; 
...
#elif defined SUSHI_450I_7U
  return 3.5;
#elif defined ARTIFICIALITY   // 人工設定 80x100:2.8
  return 2.8;
#elif defined TEST // 動作確認用テストデータ
  return 3.0;
#elif defined SAMPLE
...
#elif defined SUSHI_450I_7U
  return 5.0;
#elif defined ARTIFICIALITY   // 人工設定 80x100:4.0
  return 4.0;
#elif defined TEST // 動作確認用テストデータ
  return 5.0;
#elif defined SAMPLE
...
#elif defined SUSHI_450I_7U
  return 3.0;
#elif defined ARTIFICIALITY   // 人工設定 80x100:2.5
  return 2.5;
#elif defined TEST // 動作確認用テストデータ
  return 3.0;
#elif defined SAMPLE
