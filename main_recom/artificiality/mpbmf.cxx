...
const std::string METHOD_NAME="MPBMF";
...
int main(int argc, char *argv[]){
  ...
  //mpbmfのパラメータでループ
  int Scale = 2; //ファイル外で指定が丸いか？
  for(double mpbmf_num= 4 ; mpbmf_num < 5 ; mpbmf_num++){ // 疑似嗜好値の数
  for(double mpbmf_k = din[0] ; mpbmf_k <= din[1]; mpbmf_k++){ //潜在次元数
  for(double mpbmf_beta = 0.05; mpbmf_beta < 0.06; mpbmf_beta += 0.1){
  for(double mpbmf_alpha = 0.001; mpbmf_alpha >= 0.001; mpbmf_alpha /= 10){
    std::vector<double> para = {mpbmf_k, mpbmf_beta, mpbmf_alpha, mpbmf_num};
  ...
  if(cin[0] > 0){
    if(recom.loadSEEDandAverage(dirs[0]) == 1){ //前の実験でNaNが出ていたら1が返され，そのパラメータはスキップ
      ...
      std::cout << "K: " << mpbmf_k << "%, beta = " << mpbmf_beta << ", alpha = " << mpbmf_alpha << " is skipped." << std::endl;
      break;
    }
  }
  //時間計測
  ...
  for(recom.current() = 0; recom.current() + recom.current_start() <= recom.current_end(); recom.current()++){
    ...
    //MPBMF: 疑似嗜好値の数, Scalingpara, 潜在次元, 正則化, 学習率, 更新回数上限(指定無いと2000)
    if(recom.mpbmf_pred(dirs[0], mpbmf_num, Scale, mpbmf_k, mpbmf_beta, mpbmf_alpha, 2000) == 1){
      mpbmf_nan = true;
      recom.SeedSet2();
      break;
    }
    ...
    */
  recom.saveSEEDandAverage(dirs[0], time, mpbmf_nan);
  firstKESSONSeed_main = recom.FIRST_KESSON_SEED();
  std::cout << "K = " << mpbmf_num << "D = " << mpbmf_k << ", beta = " << mpbmf_beta << ", alpha = " << mpbmf_alpha << " is done." << std::endl;
  // if(!mpbmf_nan)
