  ...
  for(int clusters_number=4;clusters_number<=4;clusters_number+=10){
    ...
    for(double m=0.1;m<=1.0;m+= 0.2){
	  ...
	  QRFCM test(user_number, user_number, 
		clusters_number, 1, m+1, lambda);
	  std::vector<double> parameter= {lambda, m+1}; //= {lambda, m, correction}
