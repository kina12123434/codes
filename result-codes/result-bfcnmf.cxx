...
int main(){
//
const std::string METHOD_NAME="BFCNMF";
...
//読み込み
  ...
  Matrix paraMAE(12,5,0.0); //kesson x 3para
  Matrix AUCdata(12,5,0.0),tmp_AUCdata(12,5,0.0); 
  Matrix paraAUC(12,5,0.0);
  int count;
  int i; //dataのカウント
  ...
  std::string str, tmp;
  //出力ファイル名
  std::string filenameMAEout = "../../../RESULT/MAE/" + METHOD_NAME+ "_artificiality" + "MAE.txt";
  std::string filenameAUCout = "../../../RESULT/AUC/" + METHOD_NAME+ "_artificiality" + "AUC.txt";


  for(int kesson = 500 ; kesson <= 5000 ; kesson += 1500){ 
  for(int lam = 10 ; lam <= 10 ; lam *= 10){
    lam = 100000;
  ...
  std::string filenameMAE = "../../../RESULT/" + METHOD_NAME + "-80-100-0718/" + METHOD_NAME+ "_artificiality" + std::to_string(kesson)
                            + "/" + std::to_string(D) + "_" + std::to_string(c_num) + "_" + "1.797693135e+308" + "_" + EM 
                            + "/" + METHOD_NAME + "MAE.txt";
