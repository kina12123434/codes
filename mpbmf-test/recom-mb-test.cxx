...
int Recom::mpbmf_pred(std::string dir, double K_percent, double beta, double alpha, int steps) //K以外は初期値有なので指定無しでも可
{
  //K=モデルに導入された疑似嗜好値,D=潜在次元数
  Tensor R(K,return_user_number(), return_item_number()), C(K,return_user_number(), return_item_number());
  Tensor Z(K,return_user_number(), return_item_number());
  ...
  Tensor U(K,return_user_number(), D), V(K,return_item_number(), D);
  ...
  for(int rand_mf_trial = 0; rand_mf_trial < trialLimit; rand_mf_trial++){
    ...
    //Ckの生成
    double n = return_user_number();//userとitemで数の少ない方？σの大きさによる
    Tensor C_prev(C.heights(),C.rows(), C.cols());
    //Scaleは外部から取り入れる
    for(k=0; k < C.heights() ;k++){
      for(int i = 0; i < P.rows(); i++){
        for(int j = 0; j < SparseIncompleteData[i].essencialSize(); j++){ //essencialSizeである必要性はわからぬ
          C_prev[k][i][j]=std::log(1.0+SparseIncompleteData[i].elementIndex(j))/pow(Scale,K-k)//MFのRij=fijとなる
        }
      }
    }
    ...
    double error = 0.0;
    //double prev_error = DBL_MAX; \\debug
    Tensor prev_U(U.heights(), U.rows(), U.cols(), 0.0);
    Tensor prev_V(V.heights(), V.rows(), V.cols(), 0.0);
    Vector w[k];
    double sum_N;
    bool ParameterNaN = false;
    for(int step = 0; step < steps; step++){
      for(k=0; k < C.heights() ;k++){
        for(int i = 0; i < P.rows(); i++){
          for(int j = 0; j < SparseIncompleteData[i].essencialSize(); j++){
            ...
            sum_N = 0.0;
            for(k=0; k < C.heights() ;k++){
              sum_N += w[k]*1.0/(2.0*pow(std::numbers::pi,n/2.0)*sigma)
                            *std::exp(-1.0/(2.0*pow(sigma*2))*(C[k][i][j]-U[k][i]*Q[k][SparseIncompleteData[i].indexIndex(j)]));
            }
            Z[k][i][j] = w[k]*1.0/(2.0*pow(std::numbers::pi,n/2.0)*sigma)
                            *std::exp(-1.0/(2.0*pow(sigma*2))*(C[k][i][j]-U[k][i]*Q[k][SparseIncompleteData[i].indexIndex(j)]))
                          /sum_N;
            ...
            if(SparseIncompleteData[i].elementIndex(j) != 0){
              ...
              //更新式で更新
              for(int d=0;d<D;d++){
                Verr = V[k][]*Vt[k][i][d]/*転置する必要あり*/;
                U[k][i][d]
                  += alpha * (2 * Z[k][i][j] * (*U[k][i][d]-C[k][i][j]*V[k][SparseIncompleteData[i].indexIndex(j)][d]) + -U[k][i][d]/sigma_u^2);

              //Mstep　全てのZ,UVkが計算された後、に計算する。計算済みの部分だけ足し続ける予定
              w[k] += Z[k][i][j]; //Iij=1のときのみの条件必要
              sigma += Z[k][i][j]*(C[k][i][j]-U[i] * V[SparseIncompleteData[i].indexIndex(j)]);
              sigma_u += pow(U[k][i][d],2)/(return_user_number()*K); //足し合わせたらフロベニウスノルムになる
              sigma_v += pow(V[k][i][d],2)/(return_item_number()*K);
              }
              for(int k_i = 0; k_i < K; k_i++){
                P[i][k_i]
                  += alpha * (-1.0/sigma^2* err * Q[SparseIncompleteData[i].indexIndex(j)][k_i] - beta * P[i][k_i]);
                Q[SparseIncompleteData[i].indexIndex(j)][k_i]
                  += alpha * (2 * err * P[i][k_i] - beta * Q[SparseIncompleteData[i].indexIndex(j)][k_i]);
                if(!isfinite(P[i][k_i])){
                  std::cerr << "P[" << i << "][" << k_i << "] is not finite. step = " << step
      ...
      for(int i = 0; i < P.rows(); i++){
        for(int j = 0; j < SparseIncompleteData[i].essencialSize(); j++){
          if(SparseIncompleteData[i].elementIndex(j) != 0){
          error += pow(
                    SparseIncompleteData[i].elementIndex(j)//r
                    - (P[i] * Q[SparseIncompleteData[i].indexIndex(j)])
                    , 2);
          }
        }
