...
/**/
double tensor_mulCV(const Tensor &lhs, const Tensor &rhs, int k , int d, int j){
  // この関数では、計算結果のベクトルRのRijが出力される
  // プログラム上でDxNがNxDとなってるため，rowsではなくcols
  // k=嗜好数K, D=潜在次元数d
  double result = 0.0;
  //j行(固定)xN i列目x i行目xd列(固定)
    for(int i=0;i<lhs.cols();i++){
      result +=lhs[k][j][i]*rhs[k][i][d];
    }
  return result;
}

double tensor_mulUC(const Tensor &lhs, const Tensor &rhs, int k , int d, int j){
  // この関数では、計算結果のベクトルRのRijが出力される
  // プログラム上でDxNがNxDとなってるため，rowsではなくcols
  // k=嗜好数K, D=潜在次元数d
  double result = 0.0;
